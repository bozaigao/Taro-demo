...
function getRootStack({ pageList, Taro, navigationOptions }) {
    ...
    pageList.forEach(v => {
        const pageKey = v[0];
        const Screen = v[1];
        RouteConfigs[pageKey] = getWrappedScreen_1.default(Screen, Taro, navigationOptions);
    });
    return createStackNavigator(RouteConfigs, { headerLayoutPreset: 'center' });
}
exports.getRootStack = getRootStack;
...
exports.getRootStackPageList = getRootStackPageList;
...
exports.getTabRouteConfig = getTabRouteConfig;
    ...
    return createBottomTabNavigator(RouteConfigs, {
        ...
        navigationOptions: ({ navigation }) => ({
            tabBarIcon: ({ focused, tintColor }) => {
                ...
                return (react_1.default.createElement(TabBarIcon_1.TabBarIcon, { focused: focused, iconConfig: iconConfig, isRedDotShow: isRedDotShow, badgeText: badgeText, isBadgeShow: isBadgeShow, selectedIconPath: selectedIconPath || iconConfig.selectedIconPath, iconPath: iconPath || iconConfig.iconPath }));
            },
            tabBarLabel: (() => {
                const { routeName } = navigation.state;
                const tabBarIndex = tabBar.list.findIndex(item => item.pagePath === routeName) + 1;
...
const initRouter = (pageList, Taro, { window = {}, tabBar }) => {
    const navigationOptions = utils_1.getNavigationOptions(window);
